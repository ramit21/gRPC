...
public class GrpcClient {
	...
	public static void main(String[] args) {
		...
		ManagedChannel channel = ManagedChannelBuilder.forAddress("localhost", 50051)
				.usePlaintext() // to bypass HTTPS, don't do this
				.build();
	...
	private static void makeUnaryCall(ManagedChannel channel) {
		...
		Greeting greeting = Greeting.newBuilder().setFirstName("Ramit").setLastName("Sharma").build();
	...
	private static void makeServerStreamingCall(ManagedChannel channel) {
		...
		greetClient.greetManytimes(greetRequest).forEachRemaining(greetResponse -> {
			System.out.println(greetResponse.getResult());
		});
	...
	private static void makeClientStreamingCall(ManagedChannel channel) {
		...
		Arrays.asList("Ramit", "Sharma", "Kohli", "Rohit", "KL").forEach(name -> {
			requestObserver
					.onNext(GreetRequest.newBuilder().setGreeting(Greeting.newBuilder().setFirstName(name)).build());
		});
	...
	private static void makeUnaryCallWithDeadline(ManagedChannel channel) {
		...
		try {
			...
			GreetResponse greetResponse = greetClient
					.withDeadline(Deadline.after(500, TimeUnit.MILLISECONDS)) // change it to 100 to cause deadline exception
					.greetWithDeadline(greetRequest);
			System.out.println(greetResponse.getResult());
		} catch (StatusRuntimeException e) {
			if (Status.DEADLINE_EXCEEDED.getCode().equals(e.getStatus().getCode())) {
				System.err.println("Deadline breached");
			} else {
				e.printStackTrace();
			}
		}
